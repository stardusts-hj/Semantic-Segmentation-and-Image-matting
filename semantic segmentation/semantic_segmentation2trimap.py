...
def find_people(seg_map):
    ...
    y = np.expand_dims(x, 2).repeat(3,
                                    axis=2)  # expand the dimensions, seg_map (axis_x, axis_y, 1)  --->  (axis_x, axis_y, 3)
    # x = x/(15*255)
    y = y / 15
    return y, x
...
def output_seg_map(image, seg_map, original_size):
    seg_people, alpha_seg = find_people(seg_map)
    seg_image = np.multiply(seg_people, image)
    PIL_seg = Image.fromarray(np.uint8(seg_image)).convert('RGB')
    alpha_seg = alpha_seg / 15 * 255
    PIL_seg_alpha = Image.fromarray(np.uint8(alpha_seg))

    ori_PIL_seg = PIL_seg.resize((original_size[0], original_size[1]))
    ori_PIL_seg_alpha = PIL_seg_alpha.resize((original_size[0], original_size[1]))

    alpha = ori_PIL_seg_alpha
    trimap = generate_trimap(alpha)
    trimap = Image.fromarray(trimap)
    return ori_PIL_seg, ori_PIL_seg_alpha, trimap
...
def run_segmentation_trimap(image_base, pic_name, directory):
    """ implement semantic segmentation on local picture """
    for i in pic_name:
        name = i
        im_path = image_base + directory[0] + name + '.png'
        try:
        ...
        ori_PIL_seg, ori_PIL_seg_alpha, trimap = output_seg_map(resized_im, seg_map, original_size)

        ori_seg_path = image_base + directory[1] + name + '.png'
        ori_alpha_path = image_base + directory[2] + name + '.png'
        trimap_path = image_base + directory[3] + name + '.png'
        ori_PIL_seg.save(ori_seg_path)
        ori_PIL_seg_alpha.save(ori_alpha_path)
        trimap.save(trimap_path)
...
if __name__ == "__main__":
    ...
    directory = ['people/', 'people_seg/','people_alpha/','people_trimap/']
    run_segmentation_trimap(image_base, pic_name, directory)
